...
export const animalRouter = (router: Router) => {
    ...
    router.post('/animals', async (req: Request, res: Response) => {
        try {
            ...
            const insertAnimal = `insert into animal values ('${fk_costumer_cpf}','${name}','${species}','${breed}')`
    ...
    router.get('/animals/:cpf', async (req: Request, res: Response) => {
        try {
          ...
          const cpfAlreadyRegistrered = `SELECT * from costumer where fk_costumer_cpf='${cpf}'`
          ...
          const insertCostumerQuery = `select * from animal where cpf='${cpf}'`
      ...
      router.delete('/animals/:id', async (req: Request, res: Response) => {
        try {
          ...
          const cpfAlreadyRegistrered = `SELECT * from costumer where id='${id}'`
          const cpfExists = await (await client.query(cpfAlreadyRegistrered)).rows[0]
          if (!cpfExists) {
            return res.status(400).json({ message: 'O ID informado não existe' })
          }
          const deleteCostumerQuery = `delete from costumer where id='${id}'`
